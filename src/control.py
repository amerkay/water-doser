...
class Control:
    ...
    def execute_watering(self, duration_ms):
        ...
        if Logger.LOGGER_LEVEL < 2:
            ...
            log(
                "FAKE: Water ON, wait {}ms, OFF on PIN {}".format(duration_ms, pin),
                title="execute_watering",
            )
    ...
    def get_peripheral_cached(self, peripheral_name):
        ...
        if cache.is_cached(cache_id):
            log(
                "hit; using cached result: {}".format(cache.get(cache_id)),
        ...
        try:
            out = next(
                item for item in res if item["label"].lower() == peripheral_name.lower()
            )
        except:
            raise Exception(
                "Cannot find peripheral_name {} in results {}".format(
                    peripheral_name, res
                )
            )
